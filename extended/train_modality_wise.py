...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        current_valid_f1 = valid_metrics[1]

        if current_valid_f1 > best_valid_f1:
            old_best = best_valid_f1
            ...
            data_str = (
                f"Valid F1 improved from {old_best:2.4f} to {best_valid_f1:2.4f}. "
        ...
        data_str = f"Epoch: {epoch+1:03}/{num_epochs} | Epoch Time: {epoch_mins}m {epoch_secs}s | LR: {current_lr:.8f}\n"
        ...
        data_str += f"\t Best Val. F1: {best_valid_f1:.4f} at epoch {best_epoch}\n"
        print_and_save(train_log_path, data_str)

    final_str = f"Training completed.\nBest validation F1: {best_valid_f1:.4f} at epoch {best_epoch}\nBest checkpoint saved to: {checkpoint_path}\n"
    print_and_save(train_log_path, final_str)
