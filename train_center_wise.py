...
TEACHER_FILE_RULES = {
    ...
    "ADAPTIVE_FREQ_VAL": {
        "aliases": [
            "ugel_adaptive_freqampmix_val_threshold"
        ],
        "keyword_groups": [
            ["ugel", "adaptive", "freqampmix", "val", "threshold"]
        ]
    },
    "BANDHEAD_BASE": {
        "aliases": [
...
CENTER_CONFIG = {
    ...
    "BKAI": {
        "initial_teacher": "ADAPTIVE_FREQ_VAL",
        "teachers": [
            ("ADAPTIVE_FREQ_VAL", 0.45),
            ("RAHS", 0.35),
            ("WAVELET_EMA", 0.20)
        ],
        "soft_kd_weight": 0.18,
...
def should_reject_bandhead_candidate(path):
    ...
    return any(token in basename for token in reject_tokens)
...
def resolve_teacher_checkpoint(directory, prefix, teacher_key):
    ...
    for candidate in candidate_files:
        ...
        if teacher_key == "BANDHEAD_BASE":
            if should_reject_bandhead_candidate(candidate):
                continue
...
def run_experiment(path):
    ...
    config = CENTER_CONFIG[current_center]
    ...
    teacher_directory = f"files/center_wise/{model_name}"
    teacher_prefix = f"{current_center}_{current_modality}"
